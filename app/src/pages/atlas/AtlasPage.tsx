...
const WA = "https://wa.me/573185772152?text=Hola%2C%20quiero%20solicitar%20una%20demostraci%C3%B3n%20de%20Atlas.";
...
export default function AtlasPage() {
  ...
  useEffect(() => {
    ...
    document.querySelector('meta[property="og:url"]')?.setAttribute("content", "https://somacoretech.com/productos/atlas");
    document.querySelector('link[rel="canonical"]')?.setAttribute("href", "https://somacoretech.com/productos/atlas");
    ...
    let schema = document.querySelector<HTMLScriptElement>("#atlas-schema"); if (!schema) { schema = document.createElement("script"); schema.id = "atlas-schema"; schema.type = "application/ld+json"; document.head.appendChild(schema); }
    schema.textContent = JSON.stringify({ "@context": "https://schema.org", "@type": "SoftwareApplication", name: "Atlas", applicationCategory: "BusinessApplication", operatingSystem: "Web", description: t.meta.description, url: "https://somacoretech.com/productos/atlas", creator: { "@type": "Organization", name: "SOMA Core", taxID: "902080602-8" } });
  ...
  return <div className="atlas-site"><Analytics />
    <header className="atlas-header"><div className="atlas-utility"><div className="container"><a href="/">{t.back}</a><span>{t.signature}</span></div></div><div className="container atlas-nav">
      <a className="atlas-brand" href="#atlas-hero" aria-label="Atlas, inicio"><img src="/atlas-owl-hd.webp" alt=""/><span><strong>ATLAS</strong><small>{t.signature}</small></span></a>
      <nav aria-label={locale === "es" ? "Navegación Atlas" : "Atlas navigation"}>{t.nav.map((label, i) => <button key={label} onClick={() => scroll(ids[i])}>{label}</button>)}</nav>
    ...
    <main><section id="atlas-hero" className="atlas-hero"><div className="container atlas-hero-grid"><div className="atlas-hero-copy"><span className="atlas-eyebrow">{t.hero.eyebrow}</span><h1>{t.hero.title}</h1><p>{t.hero.copy}</p><div className="atlas-actions"><a className="atlas-button" href={WA}>{t.hero.primary}<ArrowRight size={18}/></a><a className="atlas-button secondary" href="mailto:contacto@somacoretech.com"><Mail size={18}/>{t.hero.secondary}</a></div><button className="atlas-text-action" onClick={() => scroll("demo")}>{t.hero.demo}<ArrowDown size={17}/></button></div>
      ...
      <section id="demo" className="atlas-section atlas-demo-section"><div className="container"><SectionHead {...t.sections.demo}/>{demoReady ? <Suspense fallback={<div className="atlas-demo-placeholder" aria-busy="true">ATLAS</div>}><AtlasDemo locale={locale}/></Suspense> : <div className="atlas-demo-placeholder"><img src="/atlas-owl-hd.webp" alt=""/><span>ATLAS</span></div>}</div></section>
      ...
      <section id="contacto" className="atlas-cta"><div className="container"><img src="/atlas-owl-hd.webp" alt=""/><div><span>{t.signature}</span><h2>{t.cta.title}</h2><p>{t.cta.copy}</p></div><div className="atlas-cta-actions"><a className="atlas-button light" href={WA}>{t.cta.whatsapp}<ArrowRight size={18}/></a><a href="mailto:contacto@somacoretech.com">{t.cta.email}</a></div></div></section></main>
