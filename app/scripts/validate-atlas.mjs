...
assert.match(demo, /Administración/);
assert.doesNotMatch(demo, /Usuarios y roles/);
assert.match(demo, /Auditoría/);
assert.match(demo, /Catálogos/);
assert.match(demo, /atlas-bi-side \$\{mobileOpen \? "open"/);
const css = read("src/pages/atlas-bi/demo/atlasBiDemo.css");
assert.match(css, /color-scheme:\s*dark/);
assert.match(css, /\.atlas-bi-app\{[^}]*position:relative/);
assert.match(css, /\.atlas-bi-side\{position:absolute;[^}]*inset:38px auto 0 0/);
assert.match(css, /\.atlas-bi-backdrop\{display:block;position:absolute;inset:38px 0 0/);
